...
public class ThemedIcon : Image {
  ...
  // Constructor paints an icon according to the current theme. ----------------
  public ThemedIcon(Theme theme, string caption, string icon_name) {

    var layers = theme.active_slice_layers;
    ...
    foreach (var layer in layers) {
      ...
      if (layer.visibility == SliceLayer.Visibility.ANY ||
        ((caption != "") == (layer.visibility == SliceLayer.Visibility.WITH_CAPTION))) {
        ...
        if (layer.layer_type == SliceLayer.Type.ICON) {
          ...
          Image text = new RenderedText(caption, layer.width, layer.height, layer.font, layer.color, theme.global_scale);
